    ...
    <div class="content-wrapper">
        ...
        <div class="border border-blue-100 mb-3 rounded-lg p-2">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-blue-600">SỬA ORDER</h2>
        ...
        <div class="border border-blue-100 rounded-lg p-3">
            <form action="{{ route('order.update', $order->id) }}" method="POST">
                @csrf
                @method('PUT') <!-- Thêm phương thức PUT cho update -->
                    ...
                    <div class="col-span-2">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="phone" id="phone"
                                value="{{ old('phone', $order->phone) }}" placeholder="Số điện thoại"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" name="email" id="email"
                                value="{{ old('email', $order->email) }}" placeholder="Email"
                                class="w-full border border-gray-300 rounded-lg p-2">
                            @error('email')
                                <div class="text-red-500">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-span-1">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="address" id="address"
                                value="{{ old('address', $order->address) }}" placeholder="Địa chỉ"
                                class="w-full border border-gray-300 rounded-lg p-2">
                            @error('address')
                                <div class="text-red-500">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="w-full border border-gray-300 rounded-lg p-2">
                                <option value="pending"
                                    {{ old('status', $order->status) == 'pending' ? 'selected' : '' }}>Chờ xử lý
                                </option>
                                <option value="processing"
                                    {{ old('status', $order->status) == 'processing' ? 'selected' : '' }}>Đang xử lý
                                </option>
                                <option value="shipped"
                                    {{ old('status', $order->status) == 'shipped' ? 'selected' : '' }}>Đang giao hàng
                                </option>
                                <option value="completed"
                                    {{ old('status', $order->status) == 'completed' ? 'selected' : '' }}>Hoàn thành
                                </option>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="note" id="note" placeholder="Ghi chú đơn hàng"
                        class="w-full border border-gray-300 rounded-lg p-2">{{ old('note', $order->note) }}</textarea>
                    @error('note')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="text-center mt-4">
                    <button type="submit" class="bg-rose-500 text-white px-4 py-2 rounded-md">
                        <i class="fa fa-save mr-1"></i> Cập nhật đơn hàng
                    </button>
                </div>
            </form>
        </div>
