            ...
            <div class="bg-white p-6 shadow-lg rounded-lg max-w-3xl mx-auto">
                <div class="mb-6">
                    <p class="text-lg text-gray-700 mb-2">
                        🎉 Cảm ơn bạn <span class="font-semibold text-rose-500">{{ $order->name }}</span> đã đặt hàng
                        tại
                        <strong>TyuuMei</strong>.
                    </p>
                    <p>Mã đơn hàng của bạn là: <strong>#{{ $order->id }}</strong></p>
                    <p class="text-sm text-gray-500 mt-2">
                        Chúng tôi sẽ sớm liên hệ để xác nhận và giao hàng đến bạn.
                    </p>
                </div>
                    ...
                    <ul class="space-y-2 text-gray-700">
                        ...
                        <li><strong>Trạng thái:</strong>
                            @switch($order->status)
                                ...
                                @case('confirmed')
                                    <span class="text-blue-600 font-semibold">Đã xác nhận</span>
                                @break
                ...
                <div class="mt-8">
                    <h4 class="text-xl font-semibold mb-4 text-gray-700">🛒 Chi tiết sản phẩm</h4>
                        ...
                        <tbody>
                            @foreach ($order->orderDetails as $detail)
                                @php
                                    ...
                                    $link = $product ? route('site.product_detail', $product->slug) : '#';
                                @endphp
                                <tr onclick="window.location='{{ $link }}'"
                                    class="cursor-pointer hover:bg-gray-50 transition"
                                    @if (!$product) style="pointer-events: none;" @endif>
                                    <td class="px-4 py-2 border-b">
                                        <img src="{{ asset('assets/images/product/' . ($product->thumbnail ?? 'no-image.jpg')) }}"
                                    ...
                                    <td class="px-4 py-2 border-b text-center text-sm text-gray-400 italic">
                                        {{-- Ẩn nút, thay bằng row click --}}
                                        @if (!$product)
                                            Không có
                                        @endif
                                    </td>
                ...
                <div class="mt-8 flex justify-center gap-4">
                    @if ($order->status === 'pending' && !$order->cancelled_at)
                        <form action="{{ route('order.cancel', $order->id) }}" method="POST"
                            onsubmit="return confirm('Bạn chắc chắn muốn hủy đơn hàng này?');">
                            @csrf
                            <button type="submit"
                                class="inline-flex items-center gap-2 bg-teal-500 hover:bg-rose-600 text-white font-semibold px-6 py-3 rounded-full shadow transition-all duration-200">
                                <i class="fa-solid fa-xmark-circle text-lg"></i> Hủy đơn hàng
                            </button>
                        </form>
                    @endif

                    <a href="{{ route('site.home') }}"
                        class="inline-flex items-center gap-2 bg-rose-500 text-white px-6 py-3 rounded-full hover:bg-teal-600 shadow transition-all duration-200">
