...
<head>
    ...
    <title>Quên Mật Khẩu - TyuuMei</title>
...
<body class="bg-gray-50 flex flex-col min-h-screen font-sans">
    ...
    <main class="flex-grow">
        <section class="container mx-auto px-6 py-12">
            ...
            <div class="bg-white p-8 sm:p-10 shadow-md rounded-xl max-w-md mx-auto">

                @if (session('status'))
                    <div
                        class="mb-6 p-4 bg-green-100 text-green-700 rounded-md border border-green-300 text-center font-medium">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form action="{{ route('password.email') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label for="email" class="block text-gray-700 font-medium mb-1">Nhập email của bạn</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500" />
                    </div>

                    <button type="submit"
                        class="w-full bg-teal-500 hover:bg-rose-500 text-white font-semibold py-3 rounded-lg transition">
                        <i class="fas fa-envelope mr-2"></i> Gửi link đặt lại mật khẩu
                    </button>
                </form>

                <div class="text-center text-sm text-gray-600 mt-6">
                    <a href="{{ route('login') }}" class="text-teal-500 hover:text-rose-500 font-medium">← Quay lại Đăng nhập</a>
                </div>
